...
<title>搜索 '<?php echo ($keyword); ?>' 的结果,<?php echo (C("WEB_NAME")); ?></title>
<meta name="keywords" content="<?php echo ($keyword); ?>搜索,<?php echo (C("WEB_KEYWORD")); ?>" />
<meta name="description" content="<?php echo ($keyword); ?>搜索,<?php echo (C("WEB_DESCRIPTION")); ?>..." />
...
<div id="main">
	<div class="location border_bottom"><a href="<?php echo (C("WEB_ADDRESS")); ?>">诗诗瘦身网</a> >> <a href="<?php echo U('/list-food');?>">食物热量查询</a> >> 搜索 '<?php echo ($keyword); ?>' 的结果</div> 
	...
	<div id="list_left">
		<h1 class="guidetitle">搜索 <font color="red">'<?php echo ($keyword); ?>'</font> 的结果列表</h1>
		<div class="g728"><?php echo (C("g72890")); ?></div>
		<?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "$emptys" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><dl>
				<dt><a href="<?php echo U('/view-' . $v['id']);?>"><img src="__PUBLIC__/Img/Images/<?php echo ($v["img_path"]); ?>" alt="<?php echo ($v["name"]); ?>" style="padding-top:40px;" /></a></dt>
				<dd>
					<strong class="clear"><a href="<?php echo U('/view-' . $v['id']);?>"><?php echo ($v["name"]); ?></a></strong>
					<div class="info">
						单位：<?php if($v['unit'] == G): ?>每100克<?php else: ?>每<?php echo ($v["unit"]); endif; ?><br />　　热量：<?php echo ($v["calories"]); ?>大卡<br />　　分类：<?php echo ($v["b_name"]); ?>　　
						所属：<?php echo ($v["a_name"]); ?>　　<a href="<?php echo U('/view-' . $v['id']);?>">查看详情>></a>　　
					</div>
				</dd>
			</dl><?php endforeach; endif; else: echo "$emptys" ;endif; ?>
		<div class="foodMenu"><?php echo ($page); ?></div>
	</div>
	<div id="list_right">
		<div class="right_1l">
