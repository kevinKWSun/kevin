...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制	
	protected $config  =	array('header'=>'条记录','prev'=>'&laquo; 上一页','next'=>'下一页 &raquo;','first'=>'首页','last'=>'末页','theme'=>' <div class="page"><div class="pagetotal">共%totalRow% %header% %nowPage%/%totalPage% 页</div><div class="pagelist"><ul><li>%first%</li><li>%upPage%</li><li>%linkPage%</li><li>%downPage%</li><li>%end%</li></ul></div><div class="pagego">跳转至第%go%页</div> ');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        */
        ...
        $thego = '<select style="width:auto;display:inline-block;" name="'.$this->varPage.'" onchange="go_page(this.value)">';
        for($i=1;$i<=$this->totalPages;$i++){
            $thego .= '<option value ="'.$i.'" ';
        ...
        function go_page(page){
            str = "'.$url.'";
            window.location = str.replace("__PAGE__",page+".html");
        }
        </script>';
        $pageStr     =   str_replace(
